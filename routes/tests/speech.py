...
@tests_bp.route("/speech/start", methods=["POST"])
@authenticate
def start_speech_test():
    """Start or get existing speech test."""
...
@tests_bp.route("/speech/status", methods=["GET"])
@authenticate
def get_speech_status():
    """Get speech test status."""
    ...
    test_dict["speech_completed"] = test.speech_score is not None

    return jsonify({"test": test_dict}), 200


@tests_bp.route("/speech/submit", methods=["POST"])
@authenticate
def submit_speech():
