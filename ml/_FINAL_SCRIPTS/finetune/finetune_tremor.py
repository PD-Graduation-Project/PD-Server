...
def _load_tremor_loss_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    tremor_losses = cfg.get("tremor_losses")
    if not isinstance(tremor_losses, dict):
        raise ValueError("Missing or invalid 'tremor_losses' section in config file")

    return tremor_losses
...
def finetune(
    ...
    *,
    ...
    """
    Finetune a trained model using one feedback sample from production.

    This function performs gradient updates on a single sample (batch size = 1)
    to incorporate human feedback after deployment.

    Args:
        model: Trained model to finetune.
        features: Tremor features for one user/sample.
        handedness: Handedness for one user/sample.
        movement: Movement id for one user/sample.
        ...
        per_movement: Same forward behavior as training code.
        ...
        loss_kwargs: Optional kwargs passed to CombinedLoss.
    ...
    cfg_loss_kwargs = _load_tremor_loss_config(config_path=config_path)
