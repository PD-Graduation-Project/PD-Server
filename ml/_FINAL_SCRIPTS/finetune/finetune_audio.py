...
def _load_audio_loss_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    """Load audio loss configuration from YAML file."""
    ...
    audio_losses = cfg.get("audio_losses")
    if not isinstance(audio_losses, dict):
        raise ValueError("Missing or invalid 'audio_losses' section in config file")
...
def finetune(
    ...
    *,
    ...
    """
    Finetune a trained audio model using one feedback sample.

    Args:
        model: Trained model to finetune.
        features: One audio feature vector (or batch size 1 tensor).
        ...
        loss_kwargs: Optional kwargs that override audio_losses from config.
    ...
    cfg_loss_kwargs = _load_audio_loss_config(config_path=config_path)
